...
class WebAuthn {
    ...
    public function __construct($rpName, $rpId, $allowedFormats=null, $useBase64UrlEncoding=false) {
        ...
        if (!$this->_formats || $invalidFormats) {
            // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
            // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
            throw new WebAuthnException('invalid formats on construct');
        }
    }
    ...
    public function queryFidoMetaDataService($certFolder, $deleteCerts=true) {
        ...
        // WordPress-only version for WordPress.org compliance - no fallbacks
        ...
        if ($deleteCerts) {
            foreach (\scandir($certFolder) as $ca) {
                if (\substr($ca, -4) === '.pem') {
                    $certFile = $certFolder . DIRECTORY_SEPARATOR . $ca;
                    // WordPress-only version for WordPress.org compliance
                    if (function_exists('wp_delete_file')) {
                        if (wp_delete_file($certFile) === false) {
                            // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
            // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
    ...
    private function _checkOrigin($origin) {
        ...
        // WordPress-only version for WordPress.org compliance  
        $scheme = function_exists('wp_parse_url') ? wp_parse_url($origin, PHP_URL_SCHEME) : '';
        if ($this->_rpId !== 'localhost' && $scheme !== 'https') {
            return false;
        }

        // extract host from origin
        // WordPress-only version for WordPress.org compliance
        $host = function_exists('wp_parse_url') ? wp_parse_url($origin, PHP_URL_HOST) : '';
        $host = \trim($host, '.');

...
class AndroidSafetyNet extends FormatBase {
    ...
    public function __construct($AttestionObject, AuthenticatorData $authenticatorData) {
        ...
        if (!\in_array($header->alg, array('RS256', 'ES256'))) {
            // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
            throw new WebAuthnException('invalid JWS algorithm', WebAuthnException::INVALID_DATA);
        }
    ...
    public function validateAttestation($clientDataHash) {
        ...
        if (!\is_array($certInfo) || ($certInfo['subject']['CN'] ?? '') !== 'attest.android.com') {
            // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
            throw new WebAuthnException('invalid certificate CN in JWS', WebAuthnException::INVALID_DATA);
        }
